...
options_metadata parser::load_settings(config::configuration *pconf)
{
    ...
    if (!nodeconf->network || !nodeconf->database || !nodeconf->chain || !nodeconf->node || !nodeconf->bitcoin)
    {
        return om;
    }
 ...
 */
    ...
    (
        ...
        value<config::checkpoint>(&nodeconf->bitcoin->bip9_bit0_active_checkpoint),
    ...
    (
        ...
        value<config::checkpoint>(&nodeconf->bitcoin->bip9_bit1_active_checkpoint),
    ...
    (
        ...
        PROPERTY(uint64_t, nodeconf->bitcoin->initial_block_subsidy_bitcoin),
        "The initial block subsidy in bitcoin, defaults to 50."
    )
    (
        "bitcoin.subsidy_interval",
        PROPERTY(uint64_t, nodeconf->bitcoin->subsidy_interval),
