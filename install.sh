...
if [ ! -d "$BUILD_DIR/libbitcoin-node" ]; then
    ...
    build_from_github JasonCoombs libbitcoin-blockchain master $PARALLEL ${BITCOIN_BLOCKCHAIN_OPTIONS[@]} "$@"
    build_from_github JasonCoombs libbitcoin-network master $PARALLEL ${BITCOIN_NETWORK_OPTIONS[@]} "$@"
    build_from_travis JasonCoombs libbitcoin-node master $PARALLEL ${BITCOIN_NODE_OPTIONS[@]} "$@"
else
